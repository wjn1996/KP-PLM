...
def run_evaluation(args, shuffle_data=True, model=None):
    ...
    if args.template and args.template != "":
        ...
        for idx, fact in enumerate(facts):
            ...
            # 如果有模板，则不用原始给的句子，而直接使用设计好的template（[x] xx [Y]）
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        (
            original_log_probs_list, # 每个位置的MLM预测概率
            token_ids_list,
            masked_indices_list, # 0-1向量，句子中mask的位置对应元素为1
        ) = model.get_batch_generation(samples_b) # 分词、喂入模型得到output
